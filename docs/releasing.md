...
2. Run the test/build checks locally:
   ...
   - `./scripts/build.sh`
   - `./xcodecli version`
3. Create and push an annotated tag like `v0.5.0`.
...
```bash
./scripts/release_homebrew.sh v0.5.0 --tap-dir .tmp/homebrew-tap --dry-run
```

To commit locally in the tap repo without pushing:

```bash
./scripts/release_homebrew.sh v0.5.0 --tap-dir .tmp/homebrew-tap
```

To clone the tap automatically and push the update:

```bash
HOMEBREW_TAP_GITHUB_TOKEN=... ./scripts/release_homebrew.sh v0.5.0 --push
```
