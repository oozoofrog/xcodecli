...
2. Run the test/build checks locally:
   ...
   - `./scripts/build.sh`
   - `./xcodecli version`
3. Create and push an annotated tag like `v0.3.1`.
...
```bash
./scripts/release_homebrew.sh v0.3.1 --tap-dir .tmp/homebrew-tap
```

To clone the tap automatically and push the update:

```bash
HOMEBREW_TAP_GITHUB_TOKEN=... ./scripts/release_homebrew.sh v0.3.1 --push
```
